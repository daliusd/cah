...
def markerLine(start_x, start_y, end_x, end_y, isBlack):
    lineColor = "White" if isBlack else "Black"

    m1 = scribus.createLine(start_x, start_y, end_x, end_y)
    scribus.setLineWidth(1, m1)
    scribus.setLineColor(lineColor, m1)
    scribus.setFillColor(lineColor, m1)
...
def createCell(text, roffset, coffset, w, h, marginl, margint, isBlack):
    #Create textbox
    start_x = marginl+(roffset-1)*w
    start_y = margint+(coffset-1)*h

    box=scribus.createRect(start_x, start_y, w, h)
    textBox=scribus.createText(start_x, start_y, w, h)
    ...
    #add marks where to cut
    end_x = start_x + w
    end_y = start_y + h
    ms = 2

    markerLine(start_x, start_y, start_x+ms, start_y, isBlack)
    markerLine(start_x, start_y, start_x, start_y+ms, isBlack)

    markerLine(start_x, end_y, start_x+ms, end_y, isBlack)
    markerLine(start_x, end_y, start_x, end_y-ms, isBlack)

    markerLine(end_x, start_y, end_x-ms, start_y, isBlack)
    markerLine(end_x, start_y, end_x, start_y+ms, isBlack)

    markerLine(end_x, end_y, end_x-ms, end_y, isBlack)
    markerLine(end_x, end_y, end_x, end_y-ms, isBlack)
